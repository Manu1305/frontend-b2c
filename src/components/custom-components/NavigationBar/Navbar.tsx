...
function Navbar() {
  ...
  const [Profile, setProfile] = useState<boolean>(false);
   const [hamburg, setHaburg] = useState<boolean>(false);
  function logout() {
    localStorage.clear();
    navigate("/login");
  }
    ...
    <div className="sticky z-50 top-0 w-full">
      <header className="shadow-lg py-4 px-4 sm:px-10  bg-white font-[sans-serif] min-h-[70px] max-sm:h-4:">
        <div className="flex flex-wrap items-center lg:gap-y-4 gap-y-6 gap-x-4">
          ...
          <ul
            className={`flex  lg:ml-14 lg:space-x-8 max-lg:space-y-2 max-lg:block max-lg:w-full transition ease-in-out delay-150  ${
              hamburg ? "s:hidden" : ""
            }`}
          >
